...
function ProjectTasksContent() {
  ...
  const effectiveWorkspaceSlug =
    typeof workspaceSlug === "string" ? workspaceSlug : workspace?.slug || "";
  const effectiveProjectSlug =
    typeof projectSlug === "string" ? projectSlug : project?.slug || "";
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      setLocalError(null);
      const prj = await getProjectBySlug(projectSlug as string, isAuth, effectiveWorkspaceSlug);
      if (!prj) {
        throw new Error(`Project "${projectSlug}" not found`);
      }
      if (prj.workspace?.slug) {
        cacheSlugId("workspace", prj.workspace.slug, prj.workspaceId);
      }
      ...
      const redirected = effectiveWorkspaceSlug
        ? await handleSlugNotFound(error, effectiveWorkspaceSlug, projectSlug as string, project?.id)
        : false;
      if (!redirected) {
        setLocalError(error instanceof Error ? error.message : "Failed to load project data");
      }
    }
  }, [effectiveWorkspaceSlug, projectSlug, handleSlugNotFound, project?.id, isAuth, getProjectBySlug]);
  ...
  const loadTasks = useCallback(async () => {
    ...
    try {
      ...
      if (isAuth && currentOrganizationId) {
        ...
        await getPublicProjectTasks(effectiveWorkspaceSlug, projectSlug as string, {
          ...commonFilters,
          page: currentPage,
          limit: pageSize,
        });
  ...
  useEffect(() => {
    if (!router.isReady) return;
      ...
      const currentRoute = `${effectiveWorkspaceSlug}/${projectSlug}`;
    if (routeRef.current !== currentRoute) {
      routeRef.current = currentRoute;
      loadInitialData();
    }
  }, [router.isReady, effectiveWorkspaceSlug, projectSlug, loadInitialData]);
