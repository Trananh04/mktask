...
export type TaskReportWorkspaceSummary = {
  id: string;
  name: string;
  slug?: string | null;
...
export type TaskReportProjectSummary = {
  id: string;
  name: string;
  slug?: string | null;
  workspace?: TaskReportWorkspaceSummary | null;
};

export type TaskReportTaskSummary = {
  id: string;
  title: string;
  slug?: string | null;
  status?: any;
  project?: TaskReportProjectSummary | null;
};
...
export type TaskStatusChangeRequest = {
  ...
  task?: TaskReportTaskSummary | null;
  requestedStatus?: any;
  requestedBy?: any;
  reviewedBy?: any;
};

export type TaskDailyReport = {
  id: string;
  taskId: string;
  reporterId: string;
  reportDate: string;
...
export const taskApi = {
  ...
  requestTaskStatusChange: async (
    taskId: string,
    data: { statusId: string; note?: string }
  ): Promise<TaskStatusChangeRequest> => {
    try {
      const response = await api.post<TaskStatusChangeRequest>(
        `/tasks/${encodeURIComponent(taskId)}/status-requests`,
        data
      );
      return response.data;
    } catch (error) {
      console.error("Request task status change error:", error);
      throw error;
    }
  },

  getTaskStatusChangeRequests: async (params: {
    organizationId: string;
    status?: "PENDING" | "APPROVED" | "REJECTED";
    requestedById?: string;
    date?: string;
    ...
    try {
      ...
      if (params.requestedById) query.append("requestedById", params.requestedById);
      if (params.date) query.append("date", params.date);
      const response = await api.get<TaskStatusChangeRequest[]>(`/tasks/status-requests?${query.toString()}`);
      return response.data;
    } catch (error) {
      console.error("Get task status change requests error:", error);
      throw error;
    }
  },

  reviewTaskStatusChangeRequest: async (
    requestId: string,
    data: { decision: "APPROVED" | "REJECTED"; note?: string }
  ): Promise<TaskStatusChangeRequest> => {
    try {
      const response = await api.patch<TaskStatusChangeRequest>(
        `/tasks/status-requests/${encodeURIComponent(requestId)}/review`,
        data
      );
      return response.data;
    } catch (error) {
      console.error("Review task status change request error:", error);
      throw error;
    }
  },

  createTaskDailyReport: async (
    taskId: string,
    data: {
      type: "START_OF_DAY" | "END_OF_DAY";
      reportDate?: string;
  ...
  ): Promise<TaskDailyReport> => {
    try {
      const response = await api.post<TaskDailyReport>(
        `/tasks/${encodeURIComponent(taskId)}/daily-reports`,
        data
      );
      return response.data;
    } catch (error) {
      console.error("Create task daily report error:", error);
      throw error;
    }
  },

  getTaskDailyReports: async (params: {
    organizationId: string;
    date?: string;
    reporterId?: string;
    type?: "START_OF_DAY" | "END_OF_DAY";
    ...
    try {
      ...
      if (params.status) query.append("status", params.status);
      const response = await api.get<TaskDailyReport[]>(`/tasks/daily-reports?${query.toString()}`);
      return response.data;
    } catch (error) {
      console.error("Get task daily reports error:", error);
      throw error;
    }
  },

  reviewTaskDailyReport: async (
    reportId: string,
    data: { note?: string }
  ): Promise<TaskDailyReport> => {
    try {
      const response = await api.patch<TaskDailyReport>(
        `/tasks/daily-reports/${encodeURIComponent(reportId)}/review`,
        data
      );
      return response.data;
    } catch (error) {
      console.error("Review task daily report error:", error);
      throw error;
    }
  },
