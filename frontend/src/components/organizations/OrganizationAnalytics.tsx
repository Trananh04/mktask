...
const emptyOrganizationAnalyticsData = {
  kpiMetrics: {
    totalWorkspaces: 0,
    activeWorkspaces: 0,
    totalProjects: 0,
  ...
  qualityMetrics: {
    ...
    resolvedBugs: 0,
    criticalBugs: 0,
    resolvedCriticalBugs: 0,
    bugResolutionRate: 0,
    criticalBugResolutionRate: 0,
  },
...
export function OrganizationAnalytics({ organizationId }: OrganizationAnalyticsProps) {
  ...
  const [widgets, setWidgets] = useState<Widget[]>(singleCompanyWidgets);

  const [kpiCards, setKpiCards] = useState<KPICard[]>(singleCompanyKPICards);
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        if (preferences.kpiCards) {
          if (Array.isArray(preferences.kpiCards)) {
            ...
            savedCards.forEach((pref) => {
              const defaultCard = singleCompanyKPICards.find((c) => c.id === pref.id);
              if (defaultCard) {
            ...
            singleCompanyKPICards.forEach((card) => {
        ...
        setWidgets(singleCompanyWidgets);
        setKpiCards(singleCompanyKPICards);
      }
    } else {
      setWidgets(singleCompanyWidgets);
      setKpiCards(singleCompanyKPICards);
  ...
  const dashboardData = useMemo(
    () => ({
      ...emptyOrganizationAnalyticsData,
      ...(data || {}),
    }),
    [data]
  );
  ...
  const renderWidgetContent = useCallback(
    ...
    [dashboardData, kpiCards, handleKPIOrderChange, taskStatuses, handleFetchData, t]
  );
