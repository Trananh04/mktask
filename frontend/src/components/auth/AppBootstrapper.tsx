...
export default function AppBootstrapper({ children }: AppBootstrapperProps) {
  ...
  const handleAuthCheck = useCallback(async () => {
    ...
    try {
      ...
      if (isPublicRoute) {
        ...
        return { isAuth: true, redirectPath, isOrg: true };
      }
      ...
      if (typeof checkOrganizationAndRedirect === "function") {
        ...
        if (currentOrgId && router.pathname === "/organization") {
          return { isAuth: true, redirectPath: orgRedirect, isOrg: true };
        }
        if (!currentOrgId && (orgRedirect === "/projects" || orgRedirect === "/dashboard")) {
          return { isAuth: true, redirectPath: orgRedirect, isOrg: true };
        }
      }
      return { isAuth: true, isOrg: true };
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      ...
      // Only redirect if path is different AND we're not already redirecting
      if (redirectPath && redirectPath !== router.pathname && !isRedirecting) {
