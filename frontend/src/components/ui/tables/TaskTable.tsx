...
const TaskTable: React.FC<TaskTableProps> = ({
  ...
  useEffect(() => {
    ...
    setColumnOrder((prev) => {
      ...
      if (prev.length === 0) {
        try {
          ...
          if (stored) {
            ...
            // If we have a valid stored order, use it, but always enforce fixed columns
            if (validStoredOrder.length > 0) {
              const fixedSet = new Set(fixedColumns);
              const reordered = [
                ...fixedColumns,
                ...validStoredOrder.filter((id) => !fixedSet.has(id)),
                ...newIds.filter((id) => !fixedSet.has(id)),
              ];
              return reordered;
            }
          }
      ...
      return [
        ...fixedColumns,
        ...currentOrderValid.filter((id) => !fixedSet.has(id)),
        ...newIds.filter((id) => !fixedSet.has(id)),
      ];
    });
