...
export default function Header() {
  ...
  useEffect(() => {
    const checkAIStatus = () => {
      const savedAIStatus = localStorage.getItem("aiEnabled");

      if (savedAIStatus !== null) {
        setIsAIEnabled(savedAIStatus === "true");
        return;
      }
  ...
  const getContextLevel = () => {
    ...
    if (
      pathParts?.length === 1 &&
      ["dashboard", "workspaces", "activity", "settings", "tasks", "reports"].includes(pathParts[0])
    ) {
      return "global";
    }
