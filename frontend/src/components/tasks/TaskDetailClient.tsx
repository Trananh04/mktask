...
export default function TaskDetailClient({
  ...
  const [dailyReport, setDailyReport] = useState({
    type: "START_OF_DAY" as "START_OF_DAY" | "END_OF_DAY",
  ...
  const handleStatusChange = async (item: any) => {
    ...
    try {
      if (isMember) {
        await taskApi.requestTaskStatusChange(taskId, {
          statusId: item.id,
          note: statusRequestNote.trim() || undefined,
        });
        setStatusRequestNote("");
        toast.success("Đã gửi yêu cầu đổi trạng thái đến manager");
        handleAIAutoClose();
        return;
      }
  ...
  const submitDailyReport = async () => {
    if (!dailyReport.content.trim()) {
      toast.error("Nhập nội dung báo cáo trước khi gửi");
      return;
    }

    setIsSubmittingReport(true);
    try {
      await taskApi.createTaskDailyReport(taskId, {
      ...
      toast.success("Đã gửi báo cáo đến manager");
    } catch (error) {
              ...
              <div className="space-y-2">
                ...
                {hasAccess && (
                  <div className="border-t border-[var(--border)] pt-4">
                    <div className="flex items-center justify-between mb-3">
                      <Label className="text-sm">Báo cáo công việc</Label>
                      <input
                        type="date"
                        value={dailyReport.reportDate}
                        onChange={(event) =>
                          setDailyReport((prev) => ({ ...prev, reportDate: event.target.value }))
                        }
                        className="h-8 rounded border border-[var(--border)] bg-transparent px-2 text-xs"
                      />
                    </div>
                    <div className="mb-3 grid grid-cols-2 gap-2">
                      <button
                        type="button"
                        onClick={() =>
                          setDailyReport((prev) => ({ ...prev, type: "START_OF_DAY" }))
                        }
                        className={cn(
                          "h-8 rounded border text-xs transition",
                          dailyReport.type === "START_OF_DAY"
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          setDailyReport((prev) => ({ ...prev, type: "END_OF_DAY" }))
                        }
                        className={cn(
                          "h-8 rounded border text-xs transition",
                          dailyReport.type === "END_OF_DAY"
                            ? "border-[var(--primary)] bg-[var(--primary)] text-white"
                            : "border-[var(--border)] text-[var(--foreground)] hover:bg-[var(--hover-bg)]"
                        )}
                      >
                    ...
                    <textarea
                      value={dailyReport.content}
                      onChange={(event) =>
                        setDailyReport((prev) => ({ ...prev, content: event.target.value }))
                      }
                    ...
                    <button
                      ...
                      className="mt-3 h-9 w-full rounded bg-[var(--primary)] px-3 text-sm font-medium text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
                    >
