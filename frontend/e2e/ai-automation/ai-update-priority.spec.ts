...
test.describe('AI Update Task Priority', () => {
  ...
  test('should update task priority via AI chat', async ({ page }) => {
    ...
    const detailVisible = await page.locator('[aria-label="Edit Status"]').isVisible().catch(() => false);
    const priorityVisible = await page.getByText(/High|Cao|Medium|Trung bình|Low|Thấp/i).first().isVisible().catch(() => false);
    expect(detailVisible || priorityVisible).toBe(true);
  });
});
