...
test.describe('Workspace Creation', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const workspaceReady = page.getByText(/workspaces|không gian làm việc/i).first();
    await expect(workspaceReady).toBeVisible({ timeout: 20000 });
  });
  ...
  test('should display workspaces list', async ({ page }) => {
    ...
    if (cardCount > 0) {
      ...
      const hasEmpty = await page.getByText(/no workspaces|không có không gian/i).isVisible().catch(() => false);
      const hasPage = await page.getByText(/workspaces|không gian làm việc/i).first().isVisible().catch(() => false);
