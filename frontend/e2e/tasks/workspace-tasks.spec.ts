...
test.describe('Workspace Tasks', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const hasCard = await workspaceCard.isVisible().catch(() => false);
    if (!hasCard) {
      const fallbackCard = page.locator('a[href*="/"]').first();
      const fallbackVisible = await fallbackCard.isVisible().catch(() => false);
      test.skip(!fallbackVisible, 'No workspace available');
      const href = await fallbackCard.getAttribute('href');
    ...
    await expect(page.getByText(/tasks|công việc/i).first()).toBeVisible({ timeout: 20000 });
  });
