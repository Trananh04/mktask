...
export class InvitationsService {
  ...
  async createInvitation(dto: CreateInvitationDto, inviterId: string) {
    ...
    // If user exists and is an org member, add them directly for workspace invitations only
    // Project invitations now always require acceptance
    ...
    // For project invitations, verify user is an org member but still create PENDING invitation
    if (dto.projectId) {
      if (!user) {
        throw new BadRequestException('User must be registered to be invited to a project');
      }
      const isOrgMember = await this.prisma.organizationMember.findUnique({
        where: {
          userId_organizationId: {
      ...
      if (dto.workspaceId) {
        ...
        try {
          await this.notificationsService.createNotification({
            ...
            entityType: 'Project',
            entityId: project.id,
            actionUrl: `/${project.workspace.slug}/${project.slug}`,
            createdBy: inviterId,
            priority: NotificationPriority.MEDIUM,
          });
        } catch (error) {
        ...
        try {
          await this.emailService.sendDirectAddNotificationEmail(user.email, {
            ...
            entityType: 'project',
            role: dto.role,
            entityUrl: `${process.env.FRONTEND_URL}/${project.workspace.slug}/${project.slug}`,
            organizationName: organization?.name,
          });
        } catch (error: unknown) {
          this.logger.error(
