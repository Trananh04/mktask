...
export class AdminService {
  ...
  async updateUserRole(id: string, role: Role, currentUserId: string) {
    ...
    const allowedSystemRoles: Role[] = [Role.SUPER_ADMIN, Role.MANAGER, Role.MEMBER];
    if (!allowedSystemRoles.includes(role)) {
      throw new BadRequestException('Only SUPER_ADMIN, MANAGER, and MEMBER are valid system roles');
    }
    ...
    const syncedMembershipRole = role === Role.SUPER_ADMIN ? Role.SUPER_ADMIN : role;
    await Promise.all([
      this.prisma.organizationMember.updateMany({
        where: { userId: id },
        data: { role: syncedMembershipRole },
      }),
