...
export class EmailService {
  ...
  async sendPasswordResetConfirmationEmail(
    ...
    try {
      const emailData = {
        ...data,
        userName: sanitizeHtml(data.userName),
        supportEmail: this.configService.get('SUPPORT_EMAIL', 'support@mktask.vn'),
        companyName: 'mktask',
      };

      await this.sendEmail({
        to: email,
        subject: 'Password Successfully Reset - mktask',
  ...
  async sendInvitationEmail(
    ...
    try {
      ...
      await this.sendEmail({
        to: email,
        subject: `You're invited to join ${data.entityName} - mktask`,
  ...
  async sendDirectAddNotificationEmail(
    ...
    try {
      ...
      await this.sendEmail({
        to: email,
        subject: `You've been added to ${data.entityName} - mktask`,
