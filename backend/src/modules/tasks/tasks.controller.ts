...
export class TasksController {
  ...
  getStatusChangeRequests(
    @Query('organizationId', ParseUUIDPipe) organizationId: string,
    @Query('status') status: TaskStatusChangeRequestStatus = TaskStatusChangeRequestStatus.PENDING,
    @Query('requestedById') requestedById: string | undefined,
    @Query('date') date: string | undefined,
    @CurrentUser() user: User,
  ) {
    return this.tasksService.listTaskStatusChangeRequests(user.id, {
      organizationId,
      status,
      requestedById,
      date,
  ...
  getDailyReports(
    @Query('organizationId', ParseUUIDPipe) organizationId: string,
    @Query('date') date: string | undefined,
    @Query('reporterId') reporterId: string | undefined,
    @Query('type') type: TaskDailyReportType | undefined,
    @Query('status') status: TaskDailyReportStatus | undefined,
    @CurrentUser() user: User,
  ) {
    return this.tasksService.listTaskDailyReports(user.id, {
      organizationId,
      date,
      reporterId,
      type,
  ...
  requestStatusChange(
    @Param('id') id: string,
    @Body() dto: RequestTaskStatusChangeDto,
    @CurrentUser() user: User,
  ) {
    return this.tasksService.requestTaskStatusChange(id, dto, user.id);
  }

  @Patch('status-requests/:requestId/review')
  @Roles(Role.MANAGER, Role.OWNER, Role.SUPER_ADMIN)
  reviewStatusChangeRequest(
    @Param('requestId', ParseUUIDPipe) requestId: string,
    @Body() dto: ReviewTaskStatusChangeDto,
    @CurrentUser() user: User,
  ) {
    return this.tasksService.reviewTaskStatusChangeRequest(requestId, dto, user.id);
  }
  ...
  createDailyReport(
    @Param('id') id: string,
    @Body() dto: CreateTaskDailyReportDto,
    @CurrentUser() user: User,
  ) {
    return this.tasksService.createTaskDailyReport(id, dto, user.id);
  }

  @Patch('daily-reports/:reportId/review')
  @Roles(Role.MANAGER, Role.OWNER, Role.SUPER_ADMIN)
  reviewDailyReport(
    @Param('reportId', ParseUUIDPipe) reportId: string,
    @Body() dto: ReviewTaskDailyReportDto,
    @CurrentUser() user: User,
  ) {
    return this.tasksService.reviewTaskDailyReport(reportId, dto, user.id);
  }
