...
import {
  CreateTaskDailyReportDto,
  RequestTaskStatusChangeDto,
  ReviewTaskDailyReportDto,
  ReviewTaskStatusChangeDto,
...
export class TasksService {
  ...
  async findAll(
    ...
    if (isRankSort && this.isValidUUID(scopeId)) {
  ...
  async update(id: string, updateTaskDto: UpdateTaskDto, userId: string): Promise<Task> {
    ...
    if (role === Role.MEMBER) {
      ...
      if (updateTaskDto.statusId) {
        const request = await this.requestTaskStatusChange(
          id,
  ...
  private async getTaskApprovalContext(taskId: string, userId: string) {
    const access = await this.accessControl.getTaskAccess(taskId, userId);
    const task = await this.prisma.task.findUnique({
      where: { id: access.task?.id ?? taskId },
      include: {
  ...
  private async ensureOrganizationManager(organizationId: string, userId: string) {
  ...
  private async ensureTaskManager(taskId: string, userId: string) {
    const { access, task } = await this.getTaskApprovalContext(taskId, userId);
  ...
  async requestTaskStatusChange(taskId: string, dto: RequestTaskStatusChangeDto, userId: string) {
    const { access, task } = await this.getTaskApprovalContext(taskId, userId);

    if (!access.canChange) {
      throw new ForbiddenException('Insufficient permissions to request a status change');
    }

    const requestedStatus = await this.prisma.taskStatus.findFirst({
      where: { id: dto.statusId, workflowId: task.project.workflowId },
      select: { id: true, name: true },
    });
    ...
    if (task.statusId === requestedStatus.id) {
      throw new BadRequestException('Task is already in this status');
    }

    const existingPending = await this.prisma.taskStatusChangeRequest.findFirst({
      where: {
        taskId: task.id,
    ...
    const request = existingPending
      ? await this.prisma.taskStatusChangeRequest.update({
          where: { id: existingPending.id },
          data: {
            requestedStatusId: requestedStatus.id,
            requesterNote: dto.note ? sanitizeText(dto.note) : null,
          },
          include: this.taskStatusRequestInclude(),
        })
      : await this.prisma.taskStatusChangeRequest.create({
          data: {
            taskId: task.id,
            requestedStatusId: requestedStatus.id,
            requestedById: userId,
            requesterNote: dto.note ? sanitizeText(dto.note) : undefined,
          },
          include: this.taskStatusRequestInclude(),
        });
  ...
  private taskStatusRequestInclude() {
    return {
      task: {
        select: {
          id: true,
      ...
      requestedStatus: { select: { id: true, name: true, color: true, category: true } },
      requestedBy: {
    ...
    } satisfies Prisma.TaskStatusChangeRequestInclude;
  }

  async listTaskStatusChangeRequests(
    userId: string,
    filters: {
      organizationId: string;
      status?: TaskStatusChangeRequestStatus;
    ...
    if (filters.date) {
      const date = this.normalizeReportDate(filters.date);
      const nextDate = new Date(date);
      nextDate.setUTCDate(date.getUTCDate() + 1);
      where.createdAt = { gte: date, lt: nextDate };
    }

    return this.prisma.taskStatusChangeRequest.findMany({
      where,
      include: this.taskStatusRequestInclude(),
  ...
  async reviewTaskStatusChangeRequest(
    requestId: string,
    dto: ReviewTaskStatusChangeDto,
    reviewerId: string,
  ) {
    const request = await this.prisma.taskStatusChangeRequest.findUnique({
      where: { id: requestId },
      include: this.taskStatusRequestInclude(),
    });
    ...
    await this.ensureTaskManager(request.taskId, reviewerId);

    if (request.status !== TaskStatusChangeRequestStatus.PENDING) {
      throw new BadRequestException('This request has already been reviewed');
    }

    if (dto.decision === TaskStatusChangeRequestStatus.REJECTED) {
      return this.prisma.taskStatusChangeRequest.update({
        where: { id: requestId },
        data: {
          status: TaskStatusChangeRequestStatus.REJECTED,
          reviewedById: reviewerId,
          reviewedAt: new Date(),
          managerNote: dto.note ? sanitizeText(dto.note) : undefined,
        },
        include: this.taskStatusRequestInclude(),
      });
    }

    const requestedStatus = await this.prisma.taskStatus.findUnique({
      where: { id: request.requestedStatusId },
      select: { category: true },
    });

    const [reviewedRequest] = await this.prisma.$transaction([
      this.prisma.taskStatusChangeRequest.update({
        where: { id: requestId },
        data: {
          status: TaskStatusChangeRequestStatus.APPROVED,
          reviewedById: reviewerId,
          reviewedAt: new Date(),
          managerNote: dto.note ? sanitizeText(dto.note) : undefined,
        },
      ...
      this.prisma.task.update({
        ...
        data: {
          statusId: request.requestedStatusId,
          completedAt: requestedStatus?.category === 'DONE' ? new Date() : null,
  ...
  private taskDailyReportInclude() {
    return {
      task: {
        select: {
          id: true,
    ...
    } satisfies Prisma.TaskDailyReportInclude;
  }

  async createTaskDailyReport(taskId: string, dto: CreateTaskDailyReportDto, userId: string) {
    const { access, task } = await this.getTaskApprovalContext(taskId, userId);

    if (!access.canChange) {
      throw new ForbiddenException('Insufficient permissions to report on this task');
    }

    const reportDate = this.normalizeReportDate(dto.reportDate);
    const report = await this.prisma.taskDailyReport.upsert({
      where: {
        taskId_reporterId_reportDate_type: {
          taskId: task.id,
          reporterId: userId,
          reportDate,
          type: dto.type,
      ...
      update: {
        ...
        blockers: dto.blockers ? sanitizeText(dto.blockers) : null,
      ...
      create: {
        ...
        type: dto.type,
        content: sanitizeText(dto.content),
        blockers: dto.blockers ? sanitizeText(dto.blockers) : undefined,
        progressPercent: dto.progressPercent,
      },
    ...
    await this.notifyManagers({
      ...
      message: `"${task.title}" có báo cáo mới`,
  ...
  async listTaskDailyReports(
    ...
    return this.prisma.taskDailyReport.findMany({
      where,
      include: this.taskDailyReportInclude(),
  ...
  async reviewTaskDailyReport(reportId: string, dto: ReviewTaskDailyReportDto, reviewerId: string) {
    const report = await this.prisma.taskDailyReport.findUnique({
      where: { id: reportId },
    ...
    await this.ensureTaskManager(report.taskId, reviewerId);

    return this.prisma.taskDailyReport.update({
      where: { id: reportId },
      data: {
        status: TaskDailyReportStatus.REVIEWED,
        reviewedById: reviewerId,
        reviewedAt: new Date(),
        managerNote: dto.note ? sanitizeText(dto.note) : undefined,
      },
