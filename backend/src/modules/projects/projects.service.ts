...
export class ProjectsService {
  ...
  async create(
    ...
    // Get or create the hidden default workspace for the single-company flow.
    let workspaceId = createProjectDto.workspaceId;
    if (!workspaceId) {
    ...
    while (retryCount < maxRetries) {
      try {
        return await this.prisma.$transaction(async (tx) => {
          const projectData: Omit<CreateProjectDto, 'workspaceId' | 'workflowId' | 'taskPrefix'> = {
            ...createProjectDto,
          };
          delete (projectData as Partial<CreateProjectDto>).workspaceId;
          delete (projectData as Partial<CreateProjectDto>).workflowId;
          delete (projectData as Partial<CreateProjectDto>).taskPrefix;
