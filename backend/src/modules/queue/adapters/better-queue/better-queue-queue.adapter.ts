...
type QueueProcessor<T, R = unknown> = (job: IJob<T>) => R | Promise<R>;
...
export class BetterQueueQueueAdapter<T = any> implements IQueue<T> {
  ...
  add(name: string, data: T, options?: JobOptions): Promise<IJob<T>> {
    ...
    void this.processLocallyIfReady(job);
   ...
   */
  setProcessor(processor: QueueProcessor<T>): void {
  ...
  private async processLocallyIfReady(job: BetterQueueJobAdapter<T>): Promise<void> {
    if (!this.processor || this.isPausedFlag || this.processingJobs.has(job.id)) {
      return;
    }
    ...
    try {
      ...
      job._markFailed(failure);
      this.failedJobs.push(job);
      this.eventEmitter.emit(QueueEvent.JOB_FAILED, job, failure);
    } finally {
