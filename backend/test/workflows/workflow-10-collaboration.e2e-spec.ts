...
describe('Workflow 10: Collaborative Task Discussion (e2e)', () => {
  ...
  describe('Collaborative Task Discussion', () => {
    ...
    it('Step 10: User B marks task as complete', async () => {
      ...
      expect(response.body.statusId).not.toBe(doneStatusId);
      expect(response.body.approvalRequired).toBe(true);
      expect(response.body.statusChangeRequest).toBeDefined();
      expect(response.body.statusChangeRequest.requestedStatusId).toBe(doneStatusId);
    });
